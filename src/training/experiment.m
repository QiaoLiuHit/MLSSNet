...
% -------------------------------------------------------------------------------------------------
    ...
    opts.expDir = 'data-MLSSNet-TIR'; % where to save the trained net
  ...
  %  opts = env_paths_training_TIR(opts);
    ...
    switch loss_opts.type
        case 'simple'
            ...
            net.addLayer('objective', ...
                         LogisticLoss(), ...
                         {'Sumscore', 'eltwise_label', 'eltwise_weight'}, 'objective');
    ...
    switch loss_opts.type
        case 'simple'
            net.addLayer('errdisp', centerThrErr('stride', resp_stride), ...
                         {'Sumscore','label'}, 'errdisp');
            net.addLayer('iou', IOUErrorScore('stride', resp_stride), ...
                         {'Sumscore', 'label', 'exemplar_size', 'instance_size'}, ...
                         'iou');
        otherwise
            error('Unknown loss')
    end
...
%     [imout_z, imout_x, labels, sizes_z, sizes_x] = vid_get_random_batch_TIR(...
%         db, imdb_video, batch, data_dir, sample_opts);
    if use_gpu
