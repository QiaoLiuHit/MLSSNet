% -----------------------------------------------------------------------------------------------------------------------
function [imout_z, imout_x, labels, sizes_z, sizes_x, class_label] = vid_get_random_batch(imdb, imdb_video, batch, data_dir, varargin)
...
% -----------------------------------------------------------------------------------------------------------------------
    ...
    % get absolute paths of crops locations   and class_label
    ...
    for i=1:batch_size
        ...
        ind_z = strfind(objects.z{i}.frame_path,'.');
        object.z_i_frame_path_no_suffix = objects.z{i}.frame_path(1:ind_z-1);
        ind_x = strfind(objects.x{i}.frame_path,'.');
        object.x_i_frame_path_no_suffix = objects.x{i}.frame_path(1:ind_x-1);
        if pair_types_neg(i)==HARD_NEG_PAIR
            %crops_z_string{i} = [strrep(fullfile(data_dir, objects.z{i}.frame_path), '.jpg','') '.' num2str(objects.z{i}.track_id, '%02d') '.crop.x.jpg'];
             crops_z_string{i} = [fullfile(data_dir, object.z_i_frame_path_no_suffix), '.' num2str(objects.z{i}.track_id, '%02d') '.crop.x.jpg'];
        else
            %crops_z_string{i} = [strrep(fullfile(data_dir, objects.z{i}.frame_path), '.jpg','') '.' num2str(objects.z{i}.track_id, '%02d') '.crop.x.jpg'];
            crops_z_string{i} = [fullfile(data_dir, object.z_i_frame_path_no_suffix), '.' num2str(objects.z{i}.track_id, '%02d') '.crop.x.jpg'];
        end
        
       % crops_x_string{i} = [strrep(fullfile(data_dir, objects.x{i}.frame_path), '.jpg','') '.' num2str(objects.x{i}.track_id, '%02d') '.crop.x.jpg'];
        crops_x_string{i} = [fullfile(data_dir, object.x_i_frame_path_no_suffix), '.' num2str(objects.x{i}.track_id, '%02d') '.crop.x.jpg'];
    end
    ...
    % convert all rgb image to grayscale
    for k=1:numel(crops)
        crops{1,k}=repmat(single(rgb2gray(uint8(crops{1,k}))),[1 1 3]);
    end
    ...
    for i=1:batch_size
        ...
        switch pair_types_neg(i)
            ...
            case HARD_NEG_PAIR
                ...
                neg_z = imresize(neg_z, [size(imout_z,1) size(imout_z,2)]);
               % tmp_z = aug_z(neg_z);
                tmp_z = neg_z;
%                 %test
