function [bboxes, speed] = tracker(varargin)
    ...
    p.init_gpu = true;%true;
    % Get environment-specific default paths.
    p.paths = struct();
    p.paths = env_paths_tracking(p.paths);
    p.visualzation_feature = false;
    p.visualzation_response = false;
    ...
    switch p.join.method
        ...
        case 'corrfilt'
            %MLSSNet
            p.trim_x_branch = {'br2_','join_xcorr','fin_','join_xcorr3','Sumscore'};
            p.trim_z_branch = {'br1_','join_cf','join_crop_z','join_cf3','join_crop_z3','join_conv_dimred1',...
                'join_conv_dimred2','join_br1_scale1','join_br1_scale2','join_a_deconv3','join_b_deconv3'};
            p.exemplarSize = 255;
			p.instanceSize = 255;
        otherwise
    ...
    net_z = init_net(net_z, p.gpus, false);%p.init_gpu
    ...
    for i=1:numel(p.trim_x_branch)
        remove_layers_from_prefix(net_z, p.trim_x_branch{i});  % remove br2_
    end
    ...
    for i = p.startFrame:nImgs
        if i>p.startFrame
            ...
            % visualzation feature map           
            if p.visualzation_feature && i>1
                pathSave = '/media/joe/000FA49A000EC49F/Desktop/Joe/tracker_benchmark_v1.0/trackers/MLSSNet/soccer-featuremap/';
                 net_visual.conserveMemory = 0;
                 net_visual.eval({'exemplar', x_crops(:,:,:,1)});
                 feature_out_id{1,1} = 'br1_x9'; %conv3
                 feature_out_id{1,2} = 'join_br1_Wfeat'; %conv3-structure-aware feature
                 feature_out_id{1,3} = 'br1_out'; %conv5
                 feature_out_id{1,4} = 'join_br1_Wfeat5'; %conv5-semantic-arare feature
             ...
             if p.visualzation_response && i>1
                  pathSave = '/media/joe/000FA49A000EC49F/Desktop/Joe/tracker_benchmark_v1.0/trackers/MLSSNet/birds-responsemap/';
                 response_out_id{1,1} = 'join_out3'; %socre3
                 response_out_id{1,2} = 'join_out'; %score5
                 response_out_id{1,3} = 'Sumscore'; %sumscore 
                 response = get_vars(net_x, response_out_id);
                 fig_handle = figure(1);
                 set(fig_handle, 'Position', [200, 200, 1000, 150]);
                 figure(fig_handle);
                 img = uint8(gather(x_crops(:,:,:,1)));
                 ...
                 for j=1:numel(response)
                    % axis on;
                     subplot('Position',[0.1+0.152*j 0 0.15 1]),imagesc(img);  
                     h = imagesc(imresize(sum(gather(response{1,j}(:,:,:,1)),3),[255 255]));
                     alpha(h,0.6);
                     axis off; 
                    % hold off;
                 end
