...
function [newTargetPosition, bestScale] = tracker_step(net_x, s_x, s_z, scoreId, z_out, x_crops, pad_masks_x, targetPosition, window, p)
    ...
    %add motion map lq
%     response_sz=[33 33 p.numScale];
%     motion_sigma_factor=0.6; 
%     motion_sigma = response_sz*motion_sigma_factor;    
%     motion_map=gaussian_shaped_labels(motion_sigma, response_sz);  
%     responseMaps=responseMaps.*repmat(motion_map, [1 1 p.numScale]);
